...
concrete_rpart = rpart(V9 ~ ., data = data_original, 
	control = rpart.control(xval = 10, maxdepth = 3))

library(partykit) 

plot(as.party(concrete_rpart), tp_args = list(id = FALSE))

as.party(concrete_rpart)
...
concrete_ctree = ctree(V9 ~ ., data = data_original, controls = ctree_control(maxdepth = 3))
...
plot(concrete_ctree, type = 'simple')
